...
fn bench_ingestion_latency(c: &mut Criterion) {
    ...
    for size in [256, 1024, 4096].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("process_batch", size),
            &data,
            |b, data| {
                b.iter(|| {
...
fn bench_buffer_slice(c: &mut Criterion) {
    ...
    c.bench_function("bytes_slice_4k", |b| {
        b.iter(|| {
            black_box(frozen.slice(0..4096))
        });
    });
    
    c.bench_function("bytes_slice_64k", |b| {
        b.iter(|| {
            black_box(frozen.slice(0..65536))
        });
    });
}
...
mod latency_gates {
    ...
    fn ingestion_latency_regression() {
        ...
        #[cfg(target_os = "linux")]
        {
            assert!(p50 < P50_LIMIT_NS, 
                "P50 regression: {} ns (limit: {} ns)", p50, P50_LIMIT_NS);
            assert!(p99 < P99_LIMIT_NS, 
                "P99 regression: {} ns (limit: {} ns)", p99, P99_LIMIT_NS);
            assert!(p999 < P999_LIMIT_NS, 
                "P999 regression: {} ns (limit: {} ns)", p999, P999_LIMIT_NS);
        }
    }
    ...
    fn zero_allocation_hot_path() {
        ...
        #[cfg(all(target_os = "linux", feature = "alloc-counter"))]
        {
            ...
            assert_eq!(allocs, 0, 
                "Hot path allocated {} times (expected 0)", allocs);
        }
    }
}
